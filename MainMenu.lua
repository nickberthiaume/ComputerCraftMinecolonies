-- MainMenu.lua
-- Dynamic screen launcher module for ComputerCraft MineColonies UI
...
MainMenu.minButtonWidth = 10
MainMenu.buttonSpacing = 2
MainMenu.menuRowStart = 3
...
function MainMenu:getWorkingDirectory()
...
function MainMenu:refreshScreenList()
    self.screens = {}
    ...
    self.message = (#self.screens > 0)
        ...
        or "No screen modules found. Add *Screen.lua files or list them in MainMenu.lua."
end

function MainMenu:buildButtons()
    self.screenButtons = {}
...
function MainMenu:drawStatusBar()
...
function MainMenu:openScreen(moduleName)
    if shell and shell.run then
        shell.run(moduleName)
    end
    self:refreshScreenList()
    self:buildButtons()
    self:draw()
...
function MainMenu:handleInput(event, side, x, y)
...
function MainMenu:run()
    ...
    while true do
        ...
        menu:handleInput(event, side, x, y)
    end
end
